...
if (isset($_POST['result'])) {
    ...
    $r_id = ucwords(mysqli_real_escape_string($conn, $_POST['r_id']));
$s_id = ucwords(mysqli_real_escape_string($conn, $_POST['s_id']));
$s_name = mysqli_real_escape_string($conn, $_POST['s_name']);
$e_name = mysqli_real_escape_string($conn, $_POST['e_name']);
$e_id = ucwords(mysqli_real_escape_string($conn, $_POST['e_id']));
$score = mysqli_real_escape_string($conn, $_POST['score']);
$t_que = mysqli_real_escape_string($conn, $_POST['t_que']);
$t_mark = mysqli_real_escape_string($conn, $_POST['t_mark']);
$status = mysqli_real_escape_string($conn, $_POST['status']);
$n_re = mysqli_real_escape_string($conn, $_POST['n_re']);
$dat = mysqli_real_escape_string($conn, $_POST['dat']);


$sql = "INSERT INTO tbl_results ( record_id, student_id, student_name, exam_name, exam_id, score, total_questions,total_mark,status,next_retake,date1)
VALUES ('$r_id', '$s_id', '$s_name', '$e_name', '$e_id', '$score', '$t_que', '$t_mark','$status','$n_re',$dat)";

if ($conn->query($sql) === TRUE) {
   echo "<script>
alert('Refersh Successfully');
window.location.href='examinations.php';
</script>";
}
 else {
  echo "<script>
alert('Refersh UnSuccessfully');
window.location.href='examinations.php';
</script>";
}
            ...
            <div class="row">
                    ...
                    <table  class="display table" style="width: 100%; cellspacing: 0;">
                        ...
                        <th>Subject</th>
                         ...
                         $stl = '<a class="btn btn-success" href="take-assessment.php?id='.$row['exam_id'].'">Take Test</a>';
                         }else{
                         $st = '<p class="text-danger">INACTIVE</p>'; 
                                               $stl = '<a class="btn btn-danger disabled" href="#">Take Test</a>';                         
                         }
                                          print '
                           <tr>
                                                <td>'.$row['exam_name'].'</td>
...
<!-- <form method="post" action=""> -->
                    ...
                    include '../database/config.php';
                    $s_id=$_SESSION['myid'];
                                           $sql = "SELECT * FROM tbl_assessment_records WHERE student_id = '$s_id' ";
                                           $result = $conn->query($sql);
                                           ...
                                           if ($result->num_rows > 0) {
                                             ...
                                             $e_name=$row['exam_name'];
                                             $e_id=$row['exam_id'];
                                             $score=$row['score'];
                                             $t_que=$row['total_questions'];
                                             $t_mark=$row['total_mark'];
                                             $status=$row['status'];
                                             $n_re=$row['next_retake'];
                                             $dat=$row['date'];
    ...
    <!--  <div class="row">
         <div class="col-md-5 col-xs-9">
            <h4 class="widget-title">Do you Want to Write Next Category Exam Please Click Here..</h4>
            ...
            <input type="hidden" name="t_que" value="<?php echo $t_que;?>"> 
            <input type="hidden" name="t_mark" value="<?php echo $t_mark;?>"> 
            <input type="hidden" name="status" value="<?php echo $status;?>"> 
            <input type="hidden" name="n_re" value="<?php echo $n_re;?>">
             <input type="hidden" name="dat" value="<?php echo $dat;?>"> 
         </div>
         <div class="col-md-2 col-xs-3">
            <button class="btn btn-success" type="submit" name="result" style="background-color: green">Refersh</button> 
         </div>
     </div>
